...
tf.app.flags.DEFINE_string(
        'model_btree', 'xception_btree', 'The name of the architecture to train.')
...
tf.app.flags.DEFINE_string(
        'distill_points', None,
        'Comma-separated list of scopes to filter the set of points to distill knowledge.')
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        ####################
        # Define the models: base and B-tree
        ####################
        def clone_fn(batch_queue):
            """Allows data parallelism by creating multiple clones of network_fn.
            """
            images, labels = batch_queue.dequeue()
            # Base model to distill...
            logits_base, end_points_base, distill_base = network_base(images)
            # B-tree model to train!
            logits_btree, end_points_btree, distill_btree = network_btree(images)

            # Distillation loss function.
            if FLAGS.distill_points is None:
                training_points = list(distill_btree.keys())
            else:
